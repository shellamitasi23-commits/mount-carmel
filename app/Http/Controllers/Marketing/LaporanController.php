...
class LaporanController extends Controller
{
    ...
    public function exportPdf(Request $request)
    {
        ...
        $title = 'LAPORAN REKAPITULASI MOUNT CARMEL';

        $data = ['type' => $type, 'date' => $date, 'title' => $title];

        switch ($type) {
            case 'lahan':
                $data['lahans'] = Lahan::with('cluster')->where('status', 'Terjual')->get();
                break;
            case 'pembeli':
                $data['pembelis'] = User::where('role', 'pembeli')->withCount('reservasis')->get();
                break;
            case 'cluster':
                $data['clusters'] = Cluster::withCount('lahans')->get();
                break;
            case 'jenazah':
                $data['jenazahs'] = Reservasi::with(['user', 'lahan.cluster'])->whereNotNull('nama_jenazah')->get();
                break;
            default:
                $data['reservasis'] = Reservasi::with(['user', 'lahan.cluster'])->latest()->get();
                break;
        }
