...
@section('title', 'Laporan Reservasi — Mount Carmel')

@section('content')
<div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
    <div>
        <h1 class="text-2xl font-bold text-slate-800 tracking-tight uppercase">Laporan Reservasi</h1>
        <p class="text-sm text-slate-500 mt-1">Data rekapitulasi reservasi dan performa pembayaran Mount Carmel.</p>
    </div>
    <a href="{{ route('accounting.laporan.pdf', array_merge(request()->all(), ['type' => 'reservasi'])) }}" 
...
<div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100 mb-6">
    <form action="{{ route('accounting.laporan.reservasi') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        <div>
        ...
        <div>
            <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Status Reservasi</label>
            <div class="relative">
        ...
        <div class="flex">
            <a href="{{ route('accounting.laporan.reservasi') }}" 
        ...
        <table class="w-full text-left text-sm whitespace-nowrap">
            <thead>
                <tr class="bg-slate-50/80 border-b border-slate-100">
                    ...
                    <th class="px-4 py-2.5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Status Reservasi</th>
                    <th class="px-4 py-2.5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Rincian Pembayaran</th>
                    <th class="px-4 py-2.5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Biaya</th>
                    <th class="px-4 py-2.5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Status Bayar</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="px-4 py-2.5">
                        ...
                        <p class="text-[10px] text-slate-400 font-bold mt-0.5">{{ $rs->lahan->cluster->nama_cluster }} ({{ $rs->lahan->tipe_lahan }})</p>
                    </td>
                    <td class="px-4 py-2.5 text-center">
                        <span class="inline-block px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-tighter
                            {{ $rs->status_reservasi == 'Disetujui' ? 'bg-emerald-50 text-emerald-600' : 
                               ($rs->status_reservasi == 'Ditolak' ? 'bg-red-50 text-red-600' : 
                               ($rs->status_reservasi == 'Selesai' ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-600')) }}">
                            {{ $rs->status_reservasi }}
                        </span>
                    </td>
                    <td class="px-4 py-2.5">
                        <p class="font-bold text-slate-900 uppercase text-[10px]">Metode: {{ ucfirst($rs->jenis_pembayaran) }}</p>
                        @php
                            $totalTerbayar = $rs->pembayarans->where('status_pembayaran', 'Lunas')->sum('jumlah_bayar');
                        @endphp
                        <p class="text-[10px] text-slate-400 font-bold mt-0.5">Terbayar: Rp {{ number_format($totalTerbayar, 0, ',', '.') }}</p>
                    </td>
                    <td class="px-4 py-2.5 font-bold text-slate-900">
                        Rp {{ number_format($rs->biaya_penuh ?? $rs->lahan->harga, 0, ',', '.') }}
                    </td>
