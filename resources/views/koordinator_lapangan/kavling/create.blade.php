<div id="createModal" class="fixed inset-0 z-50 hidden bg-slate-900/60 backdrop-blur-sm flex items-center justify-center transition-opacity">
    <div class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl transform scale-100 transition-transform overflow-hidden m-4 max-h-[90vh] overflow-y-auto">
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 sticky top-0 z-10">
            <h3 class="text-lg font-bold text-slate-800">Tambah Lahan Baru</h3>
        ...
        <form action="{{ route('koordinator_lapangan.kavling.store') }}" method="POST">
            @csrf
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1.5">Nomor Lahan <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1.5">Tipe Lahan <span class="text-red-500">*</span></label>
                ...
                <div>
                    ...
                    <select name="harga" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:border-blue-600 outline-none text-sm" disabled>
                        <option value="" disabled selected>-- Pilih Tipe Lahan Terlebih Dahulu --</option>
                    </select>
                    <p class="mt-1 text-xs text-slate-500">Pilih Tipe Lahan terlebih dahulu untuk melihat opsi harga.</p>
                </div>
...
{{-- Script untuk update dropdown harga berdasarkan Tipe Lahan --}}
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (tipeSelect && hargaSelect) {
        ...
        tipeSelect.addEventListener('change', function() {
            if (!this.value) {
                hargaSelect.disabled = true;
                hargaSelect.innerHTML = '<option value="" disabled selected>-- Pilih Tipe Lahan Terlebih Dahulu --</option>';
                return;
            }
