...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <a href="{{ route('manajer.laporan.index', ['type' => 'reservasi']) }}" class="px-4 py-3 rounded-xl text-xs font-semibold text-center transition-all {{ request('type', 'reservasi') === 'reservasi' ? 'bg-slate-900 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                <span class="material-icons-outlined text-base">shopping_cart</span>
                <div class="mt-1">Penjualan</div>
            </a>
            <a href="{{ route('manajer.laporan.index', ['type' => 'kavling']) }}" class="px-4 py-3 rounded-xl text-xs font-semibold text-center transition-all {{ request('type') === 'kavling' ? 'bg-slate-900 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                <span class="material-icons-outlined text-base">apartment</span>
                <div class="mt-1">Kavling</div>
            </a>
            <a href="{{ route('manajer.laporan.index', ['type' => 'pembeli']) }}" class="px-4 py-3 rounded-xl text-xs font-semibold text-center transition-all {{ request('type') === 'pembeli' ? 'bg-slate-900 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                <span class="material-icons-outlined text-base">people</span>
                <div class="mt-1">Pembeli</div>
            </a>
            <a href="{{ route('manajer.laporan.index', ['type' => 'cluster']) }}" class="px-4 py-3 rounded-xl text-xs font-semibold text-center transition-all {{ request('type') === 'cluster' ? 'bg-slate-900 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-5 border-b border-slate-50 flex justify-between items-center">
                ...
                <a href="{{ route('manajer.laporan.index') }}" class="text-[10px] font-bold text-blue-600 hover:text-blue-800 hover:underline uppercase tracking-widest transition-colors">Lihat Semua</a>
            </div>
