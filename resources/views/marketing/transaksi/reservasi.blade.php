...
<div class="space-y-8 animate-fade-in-up">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        <form method="GET" action="{{ route('marketing.reservasi.index') }}" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-widest mb-2">Cari Reservasi</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama pembeli, Nomor Lahan, atau status..."
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('marketing.reservasi.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-3 rounded-xl text-sm font-bold transition-all flex items-center gap-2">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="group hover:bg-slate-50/30 transition-colors">
                        ...
                        <td class="px-8 py-6 text-right">
                            ...
                            <form action="{{ route('marketing.reservasi.updateStatus', $rs->id) }}" method="POST">
                                @csrf @method('PUT')
                    ...
                    @empty
                    <tr>
                        <td colspan="{{ auth()->user()->role == 'marketing' ? '6' : '5' }}"
...
<div id="modalTambah" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm hidden flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-[2rem] w-full max-w-md p-8 shadow-2xl animate-fade-in-up">
        ...
        <form action="{{ route('marketing.reservasi.store') }}" method="POST" class="space-y-5">
            @csrf
