...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
    ...
    <a href="{{ route('marketing.laporan.pdf', array_merge(request()->all(), ['type' => 'pembeli'])) }}" 
...
{{-- Filtering --}}
<form action="{{ route('marketing.laporan.pembeli') }}" method="GET" class="relative mb-6 group">
    ...
    @if(request('search'))
    <a href="{{ route('marketing.laporan.pembeli') }}" class="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-600 transition-colors">
...
<div class="bg-white border border-slate-100 rounded-xl shadow-sm overflow-hidden">
    ...
    @if(method_exists($pembelis, 'hasPages') && $pembelis->hasPages())
